...
syn_muts <- data.frame("Sample"= NA,"Gene" = NA,"Mutation" = NA, "Type" = NA,"Syn" = NA)
...
for(maf in files){
  ...
  Mutation <- nonsyn$Genome_Change
  Type <- nonsyn$Variant_Classification
  Syn <- 'Nonsynonymous'
  nonsyn_muts <- rbind(nonsyn_muts,data.frame(Sample,Gene,Mutation,Type,Syn))
  
  
  syn <- update@maf.silent
  Sample <- substr(syn$Tumor_Sample_Barcode, 1,16)
  Gene <- paste0(syn$Hugo_Symbol,".",syn$Entrez_Gene_Id)
  Mutation <- syn$Genome_Change
  Type <- syn$Variant_Classification
  Syn <- 'Synonymous'
  syn_muts <- rbind(syn_muts,data.frame(Sample,Gene,Mutation,Type,Syn))
