#' Computes mutational load and assesses localization across grid of nerve complexes
#'
...
compute_mut_load <- function(TDAmut_object, hypermut_cutoff = NULL, min_mutload = NULL, nonsyn_muts = NULL, syn_muts = NULL, num_permutations = 1000, seed = 121) {
  ...
  if (!is.null(min_mutload)){
    min_samples <- names(mutload[mutload < 10^min_mutload])
    mut_table <- TDAmut_object@mutation_table

    mut_table <- mut_table[!(mut_table$Sample %in% min_samples),]
    TDAmut_object@mutation_table <- mut_table
  ...
  for(i in seq(1, num_complexes, 1)){
    ...
    pvals[[i]] <- rayleigh_selection(nerve_complexes[[i]], mutload_mat, num_perm = num_permutations, one_forms = FALSE, seed = seed)
  }
