...
compute_mut_load <- function(TDAmut_object, hypermut_cutoff = FALSE, nonsyn_muts = NULL, syn_muts = NULL, num_permutations = 2000, num_cores = 1) {
  
  ######## CONSOLIDATE MUTATION TABLE INTO DATA.FRAME ########
  create_mutation_mat <- function(TDAmut_object) {
    ...
    if(is.null(nonsyn_muts) || is.na(syn_muts)){
  ...
  create_mutation_mat(TDAmut_object)
 
  # Computing total mutational load
  mutload <- rowSums(TDAmut_object@syn_mutations+TDA_mutobject@nonsyn_mutations)
  ...
  if(hypermut_cutoff != FALSE){
    ...
    create_mutation_mat(mut_table)
    mutload <- rowSums(TDAmut_object@syn_mutations+TDA_mutobject@nonsyn_mutations)
  }

  TDAmut_object@mutational_load <- mutload
