...
if (max(columns)>ncol(matrix1)) 
{
  stop (paste0("Columns out of range"," Available range: 1:",ncol(matrix1)))
}


#Translating gene name to column number
...
connectivity_pvalues_table<-function (column,permutations,cores)
# Gets column range and return the table 
...
ans<-sapply(split.column,connectivity_pvalues_table(x,permutations,cores))
