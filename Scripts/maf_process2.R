...
if ( is.null(arg$scale ) ) {rescale=FALSE} else {rescale=TRUE}
...
anno<-read.csv("../../../Documents/Lab/GIT/TCGA-TDA/Annotations/Annotations.csv",as.is=T)
row.names(anno)<-anno[,1]
...
maf<-read.delim("../../../Google Drive/LAB on Drive/COAD/Mutations/hgsc.bcm.edu_COAD.IlluminaGA_DNASeq.1.somatic.v.2.1.5.0.maf",header = TRUE,as.is=T,comment.char = "#")
...
if (rescale==TRUE) {
  ...
  hypermutated<-names(which(rowSums(mat_total)>cut))
  x<-maf[maf$Tumor_Sample_Barcode %in% hypermutated,]
  rows_to_keep<-rownames(x[seq.int(1,nrow(x),by =round(scale)),])
  ...
  mat_non_syn_bin<-ifelse(mat_non_syn>0,1,0) #Non synonymous binary matrix - will be used as input for c_score
  mat_total<-mat_non_syn+mat_syn
  hist(log10(rowSums(mat_total)),breaks=20)
  }
