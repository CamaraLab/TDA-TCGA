...
num_nodes<-length(nodes) #Used for normalizing by graph size later
...
columns<-seq_along(colnames(matrix1)) #counting columns for sapply
split.column<-split(columns,ceiling(seq_along(columns)/arg$chunk)) #Splitting columns to chunks
